...
nrep <- 500L
irls_threads <- 1L
out_file <- file.path("example", "final_cox_censor60_results.rds")
  ...
  for (case in case_vec) {
    for (rr in seq_along(varX_ratio_vec)) {
      ...
      while (rep_success < nrep) {
        ...
        seed <- 2100000L + n * 1000L + match(case, case_vec) * 100000L +
          rr * 10000L + attempt
        stage <- "simulate"
...
sim_cols <- c("family", "seed", "n", "setting", "structure", "case",
              ...
              "pve_z", "pve_int", "pve_total", "var_eta", "vare", "observed_censor")
...
settings <- list(
  ...
  base_hazard = 0.10, case_vec = case_vec, rho_x = rho_x, rho_z = rho_z,
