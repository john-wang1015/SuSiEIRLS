...
fit_glmnet_cox_once <- function(dat) {
  p <- ncol(dat$X)
  q <- ncol(dat$Z)
  X_aug <- cbind(dat$X, dat$Z)
  penalty.factor <- c(rep(1, p), rep(0, q))
  fit <- glmnet::cv.glmnet(
    x = X_aug, y = dat$y, family = "cox", alpha = 1,
    penalty.factor = penalty.factor, standardize = FALSE, nfolds = 5
  )
  as.numeric(stats::coef(fit, s = "lambda.1se"))[seq_len(p + q)]
}
...
run_cox_benchmark <- function(n_rep = 10, n = 1000, p = 10,
  ...
  for (iter in seq_len(n_rep)) {
    ...
    t1 <- Sys.time()
    beta_glmnet <- tryCatch(fit_glmnet_cox_once(dat), error = function(e) e)
    elapsed <- as.numeric(difftime(Sys.time(), t1, units = "secs"))
    if (inherits(beta_glmnet, "error")) {
      rows[[row_id]] <- data.frame(
        iter = iter, method = "glmnet_cox",
        power = NA_real_, false_cs = NA_real_, n_cs = NA_integer_,
    ...
    if (inherits(fit_ibss_Z, "error")) {
      rows[[row_id]] <- data.frame(
        iter = iter, method = "IBSS_cox_Z_plus_X",
        power = NA_real_, false_cs = NA_real_, n_cs = NA_integer_,
    ...
    fit_ibss_eta <- tryCatch({
      eta_fit <- fit_cox_eta_from_z(y = dat$y, Z = dat$Z)
    ...
    if (inherits(fit_ibss_eta, "error")) {
      rows[[row_id]] <- data.frame(
        iter = iter, method = "IBSS_cox_eta_plus_X",
        power = NA_real_, false_cs = NA_real_, n_cs = NA_integer_,
