...
nrep <- 500L
irls_threads <- 1L
out_file <- file.path("example", "final_highcorr_cox_censor80_results.rds")
...
ser_cox <- logisticsusie::ser_from_univariate(surv_uni_fun)
  ...
  for (case in case_vec) {
    for (rr in seq_along(varX_ratio_vec)) {
      ...
      while (rep_success < nrep) {
        attempt <- attempt + 1L
        if (attempt > nrep + 200L) stop("Too many failed attempts in Cox simulation")
        rep_id <- rep_success + 1L
        seed <- 6200000L + n * 1000L + match(case, case_vec) * 100000L +
          rr * 10000L + attempt
        stage <- "simulate"
        ...
        ok <- tryCatch({
          ...
          Ctime <- stats::rexp(n, rate = c_rate)
          time <- pmin(Ttrue, Ctime)
          status <- as.integer(Ttrue <= Ctime)
          y <- survival::Surv(time, status)
          observed_censor <- mean(status == 0)
          ...
          add_common <- function(evals, method, time_sec) {
            ...
            evals$vare <- vare
            evals$observed_censor <- observed_censor
          ...
          fit_cv <- glmnet::cv.glmnet(
            x = X_aug, y = y, family = "cox", alpha = 1,
          ...
          add_common(eval_selected_xcs(sel, true_idx, x_cs_id, p), "glmnet_cv_1se", proc.time()[["elapsed"]] - t1)
          ...
          add_common(eval_main_index_xcs(fit_irls$discovery_summary, pip, true_idx, x_cs_id, p, coverage), "irls", proc.time()[["elapsed"]] - t1)
          ...
          pip <- fit_irls$fitX$pip[seq_len(p)]
          add_common(eval_main_index_xcs(fit_irls$discovery_summary, pip, true_idx, x_cs_id, p, coverage), "irls_fixed_sigma2_1", proc.time()[["elapsed"]] - t1)
          ...
          main_index <- ibss_x_main_index(fit_ibss, X, p, coverage)
          pip <- fit_ibss$pip[seq_len(p)]
          add_common(eval_main_index_xcs(main_index, pip, true_idx, x_cs_id, p, coverage), "ibss_hat_offset", proc.time()[["elapsed"]] - t1)
          ...
          if (fail_consecutive >= 10L) stop("Ten consecutive failed attempts in Cox simulation")
          FALSE
        })
      }
    }
...
settings <- list(
  ...
  varX_ratio_vec = varX_ratio_vec, target_censor = target_censor,
  base_hazard = 0.10, case_vec = case_vec, x_structure = x_structure,
  dim_cs = dim_cs, dim_ar = dim_ar, rho_within = rho_within,
  rho_ar = rho_ar, rho_z = rho_z,
  true_cs = true_cs,
