...
nrep <- 500L
irls_threads <- 1L
out_file <- file.path("example", "final_highcorr_nb_od2_results.rds")
...
ser_nb <- logisticsusie::ser_from_univariate(nb_uni_fun)
  ...
  for (case in case_vec) {
    for (rr in seq_along(varX_ratio_vec)) {
      ...
      while (rep_success < nrep) {
        attempt <- attempt + 1L
        if (attempt > nrep + 200L) stop("Too many failed attempts in NB simulation")
        rep_id <- rep_success + 1L
        seed <- 7200000L + n * 1000L + match(case, case_vec) * 100000L +
          rr * 10000L + attempt
        stage <- "simulate"
        ...
        ok <- tryCatch({
          ...
          t1 <- proc.time()[["elapsed"]]
          nb_hat <- fit_nb_hat_eta(y, Z)
          y_ibss <- y
          attr(y_ibss, "hat_etaZ") <- nb_hat$eta
          attr(y_ibss, "theta") <- nb_hat$theta
          fit_ibss <- logisticsusie::ibss_from_ser(
            X = X, y = y_ibss, L = L, tol = 1e-4, maxit = 100,
            num_cores = 1,
            ser_function = ser_nb
          ...
          if (fail_consecutive >= 10L) stop("Ten consecutive failed attempts in NB simulation")
          FALSE
        })
      }
    }
...
settings <- list(
  ...
  varX_ratio_vec = varX_ratio_vec, od = od, nb_mean = nb_mean, theta = theta,
  case_vec = case_vec, x_structure = x_structure, dim_cs = dim_cs,
  dim_ar = dim_ar, rho_within = rho_within, rho_ar = rho_ar,
  rho_z = rho_z,
  true_cs = true_cs,
