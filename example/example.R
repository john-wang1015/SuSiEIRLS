...
fit_scat <- SuSiE_IRLS(
  X = X, Z = Z, y = y_scat,
  family = mgcv::scat(theta = c(3, 1), min.df = 2.5),
  scale_data = FALSE, n_threads = 1,
  L = 3, min.iter = 2, max.iter = 4,
  susie.iter = 100, verbose = FALSE
)
...
fit_cox <- SuSiE_IRLS(
  X = X, Z = Z, y = survival::Surv(y_cox, status),
  scale_data = FALSE, n_threads = 1,
  L = 3, min.iter = 2, max.iter = 4,
  susie.iter = 100, verbose = FALSE
)
