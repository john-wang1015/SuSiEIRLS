...
for (r in seq_len(nsim)) {
  ...
  rows[[r]] <- data.frame(
    ...
    rv_fit = if (!is.null(fit_est$residual_variance)) {
      fit_est$residual_variance
    } else {
      NA_real_
    },
    z2_mean_all_null = mean(zz[setdiff(seq_len(p), true_idx)]),
    z2_mean_far_null = mean(zz[null_idx]),
    z2_median_far_null = stats::median(zz[null_idx]) / stats::qchisq(0.5, 1),
    min_pr = stat$min_pr,
    min_h = stat$min_h,
    med_h = stat$med_h,
    max_h = stat$max_h,
...
summary_df <- data.frame(
  metric = c("sigma2_susie", "rv_fit", "z2_mean_all_null",
             "z2_mean_far_null", "z2_median_far_null",
             "min_pr", "min_h", "med_h", "max_h", "cat_min"),
  mean = c(
    mean(res$sigma2_susie),
    mean(res$rv_fit, na.rm = TRUE),
    mean(res$z2_mean_all_null),
    mean(res$z2_mean_far_null),
    mean(res$z2_median_far_null),
    mean(res$min_pr),
    mean(res$min_h),
    mean(res$med_h),
    mean(res$max_h),
  ...
  sd = c(
    stats::sd(res$sigma2_susie),
    stats::sd(res$rv_fit, na.rm = TRUE),
    stats::sd(res$z2_mean_all_null),
    stats::sd(res$z2_mean_far_null),
    stats::sd(res$z2_median_far_null),
    stats::sd(res$min_pr),
    stats::sd(res$min_h),
    stats::sd(res$med_h),
    stats::sd(res$max_h),
    stats::sd(res$cat_min)
  ),
  q10 = c(
    stats::quantile(res$sigma2_susie, 0.1),
    stats::quantile(res$rv_fit, 0.1, na.rm = TRUE),
    stats::quantile(res$z2_mean_all_null, 0.1),
    stats::quantile(res$z2_mean_far_null, 0.1),
    stats::quantile(res$z2_median_far_null, 0.1),
  ...
  q50 = c(
    stats::quantile(res$sigma2_susie, 0.5),
    stats::quantile(res$rv_fit, 0.5, na.rm = TRUE),
    stats::quantile(res$z2_mean_all_null, 0.5),
    stats::quantile(res$z2_mean_far_null, 0.5),
    stats::quantile(res$z2_median_far_null, 0.5),
  ...
  q90 = c(
    stats::quantile(res$sigma2_susie, 0.9),
    stats::quantile(res$rv_fit, 0.9, na.rm = TRUE),
    stats::quantile(res$z2_mean_all_null, 0.9),
    stats::quantile(res$z2_mean_far_null, 0.9),
    stats::quantile(res$z2_median_far_null, 0.9),
...
saveRDS(
  ...
  file.path("example", "ordinal_ocat_residual_variance_100_results.rds")
)
