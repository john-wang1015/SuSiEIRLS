...
nrep <- 500L
irls_threads <- 1L
out_file <- file.path("example", "final_highcorr_nb_od1p5_results.rds")
...
varZ <- 0.2
varX_ratio_vec <- c(0.5, 0.25)
case_vec <- c("independent", "mediator", "interaction")
rho_ar <- 0.5
rho_cs <- 0.99
rho_z <- 0.2
x_structure <- "cs0p99_by5_ar1_0p5_by10"
r_m <- 0.25
r_i <- 0.25
ebic_gamma <- 1
glmnet_nlambda <- 50L
...
S_cs <- matrix(rho_cs, dim_cs, dim_cs)
diag(S_cs) <- 1
S_ar <- stats::toeplitz(rho_ar ^ (0:(dim_ar - 1L)))
Sx <- kronecker(S_ar, S_cs)
Hx <- chol(Sx)
...
settings <- list(
  ...
  dim_ar = dim_ar, rho_cs = rho_cs, rho_ar = rho_ar, rho_z = rho_z,
  true_cs = true_cs,
