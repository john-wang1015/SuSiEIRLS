...
fit_nb_gam <- SuSiE_IRLS(
  X = X, Z = Z, y = y_nb, family = mgcv::nb(theta = NULL),
  mgcv_model = "gam", scale_data = FALSE, n_threads = 1,
  L = 2, coverage = 0.5, min.iter = 1, max.iter = 2,
  susie.iter = 80, verbose = FALSE
)
fit_nb_bam <- SuSiE_IRLS(
...
fit_zip_gam <- SuSiE_IRLS(
  X = X, Z = Z, y = y_zip, family = mgcv::ziP(),
  mgcv_model = "gam", scale_data = FALSE, n_threads = 1,
  L = 2, coverage = 0.6, min.iter = 1, max.iter = 2,
  susie.iter = 80, verbose = FALSE
)
fit_zip_bam <- SuSiE_IRLS(
  X = X, Z = Z, y = y_zip,
  family = mgcv::ziP(theta = c(-1, 0.2), b = 0.1),
  mgcv_model = "bam", scale_data = FALSE, n_threads = 1,
  L = 2, coverage = 0.6, min.iter = 1, max.iter = 2,
  susie.iter = 80, verbose = FALSE
)
